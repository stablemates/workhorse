  ...
  const document = {
    ...
    info: {
      ...
      description:
        ...
        "Every procedure is one POST with a JSON envelope. The HTTP path does not carry the contract version: each SDK release binds its backend to the matching dashboard contract and browser bundle (ADR 0054). " +
        "A breaking change creates dashboard/v2, and its SDK release notes give operators the upgrade and deprecation notice. Deprecation and Sunset response headers do not apply because an installed backend serves one bound contract rather than negotiating concurrent versions. " +
