...
const outputPaths = {
  ...
  "rust/src/dashboard/v1_generated.rs": join(
    repositoryRoot,
    "rust",
    "src",
    "dashboard",
...
function pythonDeclarations(roots: Array<[string, Schema]>): Array<[string, string]> {
  ...
  return ordered.map(([name, schema]) => [name, pythonDeclaration(name, schema, names)]);
}

function pythonExportRank(name: string): number {
  return /^[A-Z0-9_]+$/.test(name) ? 0 : /^[A-Z]/.test(name) ? 1 : 2;
}

// `__all__` follows Ruff's RUF022 order: SCREAMING_SNAKE_CASE, then CamelCase, then snake_case.
function pythonAll(names: string[]): string {
  // oxlint-disable-next-line unicorn/no-array-sort -- ES2022 lacks Array.prototype.toSorted.
  const sorted = [...names].sort(
    (left, right) =>
      pythonExportRank(left) - pythonExportRank(right) ||
...
function generatePython(contract: DashboardContract): string {
  ...
  const declared = pythonDeclarations(roots);
  const declarations = declared.map(([, declaration]) => declaration).join("\n\n");
  const schemas = JSON.stringify(inputSchemas(contract));
  const validators = Object.keys(contract.procedures).map(
    (name) => [name, `validate_${snakeName(name)}_input`] as const,
  );
  const wrappers = validators
    .map(
      ([procedure, validator]) =>
        `def ${validator}(value: object) -> None:\n    validate_input(${JSON.stringify(procedure)}, value)`,
    )
    .join("\n\n");
  const exported = pythonAll([
...
/**
 * Rust validates with the `jsonschema` crate the core client already uses for contracts, so its
 * binding carries the procedure names and request schemas rather than a generated validator.
 */
function generateRust(contract: DashboardContract): string {
  const schemas = JSON.stringify(inputSchemas(contract));
  let hashes = "#";
  while (schemas.includes(`"${hashes}`)) hashes += "#";
  const procedures = Object.keys(contract.procedures)
...
/// Each procedure's JSON Schema 2020-12 request schema, or \`null\` when it accepts no input.
pub(crate) const INPUT_SCHEMAS: &str = r${hashes}"${schemas}"${hashes};
`;
}
...
export async function composeDashboardBindings(): Promise<Record<string, string>> {
  ...
  return {
    ...
    "rust/src/dashboard/v1_generated.rs": execFileSync(
      "rustfmt",
      ["--edition", "2021", "--emit", "stdout"],
      { cwd: repositoryRoot, input: generateRust(contract), encoding: "utf8" },
    ),
  };
}
