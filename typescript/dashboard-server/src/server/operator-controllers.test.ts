...
describe("shared dashboard operator controllers", () => {
  it("routes application and operator calls while delegating host audit plumbing", async () => {
    ...
    const admin = {
      ...
      redriveMany: vi.fn<() => Promise<unknown>>().mockResolvedValue({
        ...
        nextCursor: { finishedAt: "2026-08-12T12:05:00.000000Z", taskId: "task-5" },
      }),
    } as unknown as Admin;
    ...
    await expect(controllers.taskController.runTaskNow?.("task-1", audit)).resolves.toEqual({
      status: "released",
      id: "task-1",
      state: "ready",
      runAt: "2026-08-12T12:00:00.000Z",
    });
    await expect(controllers.taskController.cancelTask?.("task-1", audit)).resolves.toMatchObject({
      status: "canceled",
    ...
    await expect(controllers.taskController.redriveTask?.("task-1", audit)).resolves.toEqual({
      status: "redriven",
      sourceTaskId: "task-1",
      targetTaskId: "task-2",
      sourceState: "failed",
    ...
    await expect(
      controllers.taskController.redriveDeadLetters?.(
        { queue: "critical", taskType: "charge", tags: ["billing"] },
        25,
        null,
        audit,
      ),
    ...
    expect(admin.runTaskNow).toHaveBeenCalledWith("task-1", audit);
    ...
    expect(admin.redrive).toHaveBeenCalledWith("task-1", {
  ...
  it("sends only the dead-letter filters a selection actually set, and continues from a cursor", async () => {
    ...
    await expect(
      controllers.taskController.redriveDeadLetters?.(
        { queue: null, taskType: null, tags: [] },
        100,
        { finishedAt: "2026-08-12T12:05:00.000000Z", taskId: "task-5" },
        audit,
      ),
    ...
    expect(redriveMany).toHaveBeenCalledWith({}, audit, {
      limit: 100,
      cursor: { finishedAt: "2026-08-12T12:05:00.000000Z", taskId: "task-5" },
    });
  });
