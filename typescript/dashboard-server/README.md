...
imports through `@workhorse/dashboard` continue to work through the compatibility package.
...
```ts
import { createDashboardHost } from "@workhorse/dashboard-server/server";
...
```ts
import { dashboardNodeMiddleware } from "@workhorse/dashboard-server/server";
