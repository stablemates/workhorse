...
describe("schema installation", () => {
  ...
  it("installs schema v41 with database-owned settings, job contracts, and fenced progress", async () => {
    const version = await pool.query<{ version: number }>(
      "SELECT max(version)::integer AS version FROM workhorse.schema_version",
    );
    expect(version.rows[0]?.version).toBe(41);
    ...
    expect(migrations.rows).toEqual([{ version: 41, description: "pre-release baseline" }]);
