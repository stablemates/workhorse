...
import type { DashboardTaskRow } from "../../dashboard-server/src/wire.js";
...
const expected = new Map<string, DashboardTaskRow["enqueueMode"]>();
...
describe("dashboard accepted enqueue modes", () => {
  ...
  it.each(["dashboard_tasks_v1", "dashboard_tasks_cursor_v1"])(
    ...
    async (name) => {
      const result = await database.pool.query<{
        current: { tasks: DashboardTaskRow[] };
      ...
      for (const task of current.tasks) expect(task.enqueueMode).toBe(expected.get(task.id));
    },
  );
  ...
  it.each(["dashboard_tasks_v1", "dashboard_tasks_cursor_v1"])(
    ...
    async (name) => {
      const result = await database.pool.query<{ page: { tasks: unknown[] } }>(
        `SELECT workhorse.${name}('{"queue":"missing"}'::jsonb) AS page`,
      );
      expect(result.rows[0]!.page.tasks).toEqual([]);
    },
  );
});
