...
function resolveDashboardReadSurfaceOptions(
  ...
  const resolved = {
    tasks: options.tasks ?? 100_000,
    liveTasks: options.liveTasks ?? 20_000,
  ...
  if (resolved.liveTasks >= resolved.tasks) {
    throw new RangeError(
      "liveTasks must be smaller than tasks so both live and terminal reads are loaded",
    );
  }
  return resolved;
}
...
async function loadDataset(
  ...
  await pool.query(
    `INSERT INTO workhorse.task_runtime(
       task_id, queue_name, state, run_at, ready_at, sequence,
  ...
  await pool.query(
    `INSERT INTO workhorse.task_wait(
       task_id, wait_name, mode, duration_ms, wake_at, attempt,
  ...
  await pool.query(`
    ...
    ANALYZE workhorse.task_wait;
    ANALYZE workhorse.task_checkpoint;
  `);
}
...
const dashboardRelations: Record<DashboardReadSurface, Record<DashboardRelation, string>> = {
  ...
  views: {
    task: "dashboard_task_v1",
    runtime: "dashboard_task_runtime_v1",
    outcome: "dashboard_task_outcome_v1",
    wait: "dashboard_task_wait_v1",
    checkpoint: "dashboard_task_checkpoint_v1",
    event: "dashboard_task_event_v1",
...
function taskPageSql(
  ...
  return jsonRows(`
    WITH tasks AS (
      SELECT j.id, j.queue_name AS queue, j.task_type AS type,
        ...
        FROM ${relation(surface, "task")} j
        LEFT JOIN ${relation(surface, "runtime")} r ON r.task_id = j.id
        LEFT JOIN ${relation(surface, "outcome")} o ON o.task_id = j.id
...
function activityBucketsSql(
  ...
  return jsonRows(`
    WITH candidate AS (
      SELECT task_id FROM ${relation(surface, "runtime")} WHERE updated_at >= ${parameters.cutoff}
      UNION
      SELECT task_id FROM ${relation(surface, "outcome")} WHERE updated_at >= ${parameters.cutoff}
    ), tasks AS (
      SELECT COALESCE(r.worker_id, attempt_worker.worker_id, 'unassigned') AS group_key,
        ...
        FROM candidate
        JOIN ${relation(surface, "task")} j ON j.id = candidate.task_id
        LEFT JOIN ${relation(surface, "runtime")} r ON r.task_id = candidate.task_id
        LEFT JOIN ${relation(surface, "outcome")} o ON o.task_id = candidate.task_id
        LEFT JOIN LATERAL (
...
function eventFeedSql(
  ...
  const taskMatches = (column: string) => `EXISTS (
    SELECT 1 FROM ${relation(surface, "task")} task
     WHERE task.id = ${column} AND task.queue_name = ${parameters.queue}
  )`;
  const eventCondition = `event.occurred_at >= ${parameters.cutoff}
    AND ${taskMatches("event.task_id")}`;
  const attemptCondition = `history.occurred_at >= ${parameters.cutoff}
    AND ${taskMatches("history.task_id")}`;
  return jsonRows(`
    ...
    SELECT page.kind, page.record_id::text, page.task_id::text, task.queue_name, task.task_type,
           ...
           page.error
      FROM page
      LEFT JOIN ${relation(surface, "task")} task ON task.id = page.task_id
    ...
    SELECT (
      (SELECT count(*) FROM ${relation(surface, "event")} event
        WHERE event.occurred_at >= ${parameters.cutoff}
          AND EXISTS (SELECT 1 FROM ${relation(surface, "task")} task
                       WHERE task.id = event.task_id AND task.queue_name = ${parameters.queue}))
      +
      (SELECT count(*) FROM ${relation(surface, "attempt")} history
        WHERE history.occurred_at >= ${parameters.cutoff}
          AND EXISTS (SELECT 1 FROM ${relation(surface, "task")} task
                       WHERE task.id = history.task_id AND task.queue_name = ${parameters.queue}))
    )::text AS count
  `);
}
...
function taskTotalSql(includeAttemptHistory: boolean): string {
  ...
  return `WITH tasks AS (
            SELECT j.id,j.queue_name AS queue,j.task_type AS type,j.tags,
                   COALESCE(r.worker_id,current_wait.worker_id,${attemptWorker},'unassigned') worker_id
              FROM workhorse.dashboard_task_v1 j
              LEFT JOIN workhorse.dashboard_task_runtime_v1 r ON r.task_id=j.id
              LEFT JOIN workhorse.dashboard_task_outcome_v1 o ON o.task_id=j.id
              LEFT JOIN workhorse.dashboard_task_wait_v1 current_wait
                ON current_wait.task_id=j.id AND current_wait.wait_name=r.wait_name
              ${attemptJoin}
          )
...
async function readBaselineTasks(query: CountedQueryRunner): Promise<void> {
  await Promise.all([
    query
      .query({
        text: "SELECT estimate FROM workhorse.dashboard_task_estimate_v1()",
        values: [],
      })
      .then(async () => {
        await query.query({
        ...
        for (const condition of [
          ...
          await query.query({
            text: `EXPLAIN (FORMAT JSON) SELECT 1
                     FROM workhorse.dashboard_task_outcome_v1 WHERE ${condition}`,
            values: [],
          });
        }
      }),
      ...
      (["succeeded", "failed", "canceled"] as const).map((state) =>
        query.query({
          text: `EXPLAIN (FORMAT JSON)
                 SELECT 1 FROM workhorse.dashboard_task_outcome_v1 outcome
  ...
  for (const row of queueRows) {
    for (const state of ["succeeded", "failed", "canceled"] as const) {
      ...
      branches.push(`SELECT $${start}::text queue,$${start + 1}::text state
                       ...
                       JOIN workhorse.dashboard_task_v1 task ON task.id=outcome.task_id
                      WHERE task.queue_name=$${start + 2} AND outcome.state=$${start + 3}`);
...
export async function runDashboardReadSurfaceBenchmark(
  ...
  return {
    ...
    dataset: {
      tasks: options.tasks,
      liveTasks: options.liveTasks,
      terminalTasks: options.tasks - options.liveTasks,
      events: options.tasks * 2,
      attempts: options.tasks - options.liveTasks,
      waits: Math.floor(options.liveTasks / 10),
      checkpoints: Math.ceil(options.tasks / 10),
    },
    executionOrder,
    cases,
    requests,
