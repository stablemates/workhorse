 ...
 * checkout's `.env` unconditionally, falling back to the repository's documented local database
 * defaults when the file is absent. Every other variable keeps the familiar `--env-file`
...
/** Database variables this repository provisions per worktree, and so is entitled to overwrite. */
...
for (const purpose of localDatabasePurposes) {
  environment[localDatabaseEnvironmentVariable(purpose)] = localDatabaseUrl(
    purpose,
    fileEnvironment,
  );
}

if (fileEnvironment.ONTRACK_AGENT_DSN === undefined) {
  delete environment.ONTRACK_AGENT_DSN;
} else {
  environment.ONTRACK_AGENT_DSN = fileEnvironment.ONTRACK_AGENT_DSN;
}

for (const [key, value] of Object.entries(fileEnvironment)) {
  if (
    !databaseVariables.has(key) &&
    key !== "ONTRACK_AGENT_DSN" &&
