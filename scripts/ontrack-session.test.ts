...
describe("checkout Ontrack Session", () => {
  it.each([
    ...
    "postgres://feature%2Fone@pg.ontrack.sh:35500/postgres?sslmode=require",
  ])("rejects unsafe DSN %s", (dsn) => {
    expect(() => validateOntrackAgentDsn(dsn)).toThrow(/.+/);
  });

  it("normalizes a linked-worktree id without preserving credentials", () => {
    const sessionName = ontrackSessionName("feature/one");
    const rewritten = ontrackAgentDsnForSession(safeDsn, "feature/one");
    expect(sessionName).toMatch(/^feature-one-[a-f0-9]{8}$/);
    expect(rewritten).toBe(
      `postgres://${sessionName}@pg.ontrack.sh:35500/postgres?sslmode=require`,
    );
    expect(validateOntrackAgentDsn(rewritten).sessionName).toBe(sessionName);
    expect(ontrackSessionName("a".repeat(65))).toHaveLength(64);
  });
  ...
  it("copies only the primary DSN into a linked checkout and preserves its other values", async () => {
    ...
    expect(configured.sessionName).toMatch(/^feature-one-[a-f0-9]{8}$/);
    expect(contents).toContain("CHECKOUT_ONLY=yes");
    expect(contents).not.toContain("PRIMARY_ONLY=yes");
    expect(contents).toContain(`ONTRACK_AGENT_DSN=postgres://${configured.sessionName}@`);
