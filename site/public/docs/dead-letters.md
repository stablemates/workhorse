...
`queue.listDeadLetters(query)` reads a cold failure index that stays separate from the ready and
...
`queue.redrive(sourceJobId, request)` requires three things: who asked, why, and a stable request
...
`queue.getRedriveLineage(jobId)` walks the connected graph in either direction, so you can trace
...
`queue.redriveMany(filter, request, options)` processes a bounded page of matching failures in
stable order. Start with a dry run:

```ts
const preview = await queue.redriveMany(
  { queue: "billing", errorName: "ProviderTimeout" },
  { requestedBy: actor.email, reason: "provider incident resolved", requestId: incidentId },
...
do {
  const page = await queue.redriveMany(
    { queue: "billing", errorName: "ProviderTimeout" },
    { requestedBy: actor.email, reason: "provider incident resolved", requestId: incidentId },
    { limit: 100, cursor },
  );
