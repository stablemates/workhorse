...
```ts
worker.handle("catalog.import", async (payload: { source: string }, ctx) => {
  ...
  await ctx.setProgress({ phase: "complete", processed });
  return { imported: processed };
});
```

`HandlerContext.getProgress` returns the latest retained value from inside a handler. Outside
one, `Queue.getProgress` reads it by job ID, and a custom runtime can write through
