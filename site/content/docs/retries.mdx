---
title: Retries
description: Give tasks an attempt budget and a persisted backoff policy that PostgreSQL applies consistently.
---

When a handler throws, the task usually is not finished — it has failed one attempt. PostgreSQL
records the failure evidence, checks whether budget remains, and moves the task back to
...
`EnqueueOptions.maxAttempts` limits how many logical attempts one task may use. Each failure —
...
## Persist the delay policy with the task

Retrying instantly is usually wrong: if a provider is down, hammering it just burns attempts.
`EnqueueOptions.retryPolicy` stores a backoff shape with the accepted task definition, and
PostgreSQL validates it at enqueue time. Pending keyed debounce may replace that definition before
dispatch. Three shapes exist:
...
tasks fail together, jitter spreads their wake times so the retry wave cannot knock the
recovering service over again.

Workhorse derives the jitter from stable task state — identity and attempt number — rather
...
A task can need a retry for two different reasons: its handler threw, or its worker died and
...
Tasks without a policy keep older compatibility behavior, which differs between the two paths:
...
`WorkerOptions.retryDelayMs` takes a number or `(attempt, task) => number | undefined`. An
...
The task ID, payload, tags, policy, and every saved [checkpoint](/docs/durable-execution)
...
A retry is the same task having another go. A redrive is different: it happens only after
terminal failure and creates a [new task identity](/docs/dead-letters).

`TaskSnapshot.retryPolicy` shows the normalized policy, and timeline events record each
selected delay with its source. When an operator asks why a task is still `scheduled`, the
evidence answers.

## Next

- [Durable execution](/docs/durable-execution) — reuse completed work on another attempt
- [Dead letters](/docs/dead-letters) — create a fresh task after retries end
