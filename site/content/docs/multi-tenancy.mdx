...
synchronize with `Queue.sync_budgets` or `AsyncQueue.sync_budgets`. Rust applications set the same
snake_case fields and synchronize with `Queue::sync_budgets`.
