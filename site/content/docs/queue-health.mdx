...
Override budgets per call when deployment needs differ. The dashboard and `workhorse health
--json` use the same evaluation, so every operator surface agrees on the verdict.
