---
title: Drizzle
description: Adapt a node-postgres Drizzle database and enqueue tasks inside caller-owned transactions.
---
...
and a committed row with no task is a silent gap. `@stablemates/workhorse-drizzle` closes that gap: it converts
...
```bash
pnpm add @stablemates/workhorse-drizzle drizzle-orm pg
```
...
`adminForTransaction`, `createWorker`, and `close`. It gives worker processes and operator tools
one stable surface without adding Drizzle to core.
...
returned `Queue` sends its SQL through that transaction, so the task follows the surrounding commit
or rollback.
...
If the callback throws, Drizzle rolls back both the account and the task. Nothing outside the
