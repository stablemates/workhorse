...
`schedule_catchup_limit`. Both runtimes evaluate definitions only when PostgreSQL grants the
maintenance tick, so one process does the cron work for each pass.
...
alarms; prefer UTC cron expressions. If local clocks skip a scheduled time, Workhorse fires after
the clock advances. If clocks repeat a time, Workhorse fires its first occurrence only. Hashed `H`
fields also choose a stable offset across Python and TypeScript workers, so either runtime fires
the same occurrence.
