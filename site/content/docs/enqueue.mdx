...
```ts
const audit = { actor: actor.email, reason: "provider maintenance", requestId: incidentId };
await admin.pauseQueue("billing", audit); // workers stop claiming; jobs are unchanged
await admin.resumeQueue("billing", { ...audit, requestId: `${incidentId}:resume` });
await admin.purgeQueue("billing", { ...audit, requestId: `${incidentId}:purge` });
```
