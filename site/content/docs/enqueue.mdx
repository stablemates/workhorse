...
description: Accept immediate, delayed, batched, or transactional tasks without separating them from application state.
---

The classic queue bug is a task that disagrees with your data: the order row committed but the
fulfillment task was lost, or the task exists and the row does not. Workhorse removes that gap by
...
`queue.enqueue(type, payload, options?)` turns a type and JSON payload into a durable task and
returns its ID. Without `runAt`, PostgreSQL places the task in `ready` and a worker can claim it
...
become unavailable, a worker claims the task later than the boundary — never earlier.

`EnqueueOptions` carries everything else a task can be born with:
...
PostgreSQL commits the task and your business write together — or rolls both back together. There is
...
the transaction in an executor and builds a `Queue` around that. Rust builds a `Queue` on the open
...
and `NewSQLExecutor` accepts a `*sql.Tx`. Rust's `Queue::new` accepts a `tokio_postgres` client or
`Transaction`, or their `deadpool_postgres` counterparts. Python's `Worker` needs its own connection
in autocommit mode and raises `ValueError` otherwise, so keep the connection you enqueue on separate
from the one a worker runs on.

SQLAlchemy applications using its Psycopg dialect can reach that same driver connection through
`sqlalchemy_connection.connection.driver_connection`. Build the queue inside SQLAlchemy's own
transaction so it cannot outlive the application write:
...
engine = create_engine("postgresql+psycopg://localhost/application")

with engine.begin() as sqlalchemy_connection:
    sqlalchemy_connection.execute(
        text("INSERT INTO account (id, email) VALUES (:id, :email)"),
        {"id": account_id, "email": email},
    )
    psycopg_connection = sqlalchemy_connection.connection.driver_connection
    Queue(psycopg_connection).enqueue("account.created", {"accountId": account_id})
```

`driver_connection` is the Psycopg connection already enlisted in this transaction. SQLAlchemy
still owns the commit, rollback, and pool checkout.
...
If a single operation produces many tasks — a fan-out to every subscriber, an import — use
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  <Tab value="TypeScript">
    ```ts
    const taskIds = await queue.enqueueMany(recipients.map((recipient) => ({
...
`new Queue(pool, "billing")` sets the client default queue; `options.queue` overrides one task. A
...
Workhorse dispatches higher-priority tasks first, and it keeps FIFO order among tasks with the same
value. Priority is strict, so a sustained stream of urgent work can delay lower-priority tasks.
...
- [Idempotency](/docs/idempotency) — make repeated acceptance converge on one task
- [Schedules](/docs/schedules) — create recurring tasks from desired state
