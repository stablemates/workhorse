...
The problem: a request creates an order and must guarantee a fulfillment task — but only if the
order commits. Pass your open transaction as the last argument to `enqueue`, and the task and the
...
If the insert fails, no task exists. If the enqueue fails, no order exists. There is no window where
...
The sleep stores a named timer, drops the lease, and parks the task in `scheduled` — no worker slot
...
and planned time, so PostgreSQL converges every racing call on one task. Removing the definition
from the array disables it on the next deploy — code stays the owner of intent.

## Webhook idempotency

The problem: payment providers redeliver webhooks, and each delivery must produce exactly one task.
Build the idempotency key from the provider's event ID; a replayed delivery returns the original
task instead of creating another.
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  ...
  <Tab value="Go">
    ...
    func handleStripeWebhook(
    	...
    	taskID, err := queue.Enqueue(ctx, "stripe.event", map[string]any{
    		"eventId":   event.ID,
    	...
    	return map[string]string{"accepted": taskID}, nil
    }
    ```
...
An equivalent replay returns the same task ID with no new task, event, or notification — the route
...
Rust returns `Error::EnqueueIdempotencyConflict`.
...
The problem: a provider outage dead-lettered a batch of tasks, the provider has recovered, and an
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  <Tab value="TypeScript">
    ...
    console.log(`${preview.results.length} tasks eligible`);
    ...
    do {
      ...
      for (const result of page.results) {
        console.log(result.status, result.sourceTaskId, "->", result.targetTaskId);
      }
  ...
  <Tab value="Python">
    ...
    def redrive_incident(admin: Admin) -> None:
        ...
        print(f"{len(preview.results)} tasks eligible")
        ...
        while True:
            ...
            for result in page.results:
                print(result.status, result.source_task_id, "->", result.target_task_id)
    	...
    	for {
    		...
    		for _, result := range page.Results {
    			...
    			if result.TargetTaskID != nil {
    				target = *result.TargetTaskID
    			}
    			fmt.Println(result.Status, result.SourceTaskID, "->", target)
    		}
...
Each redrive creates a fresh task that copies the queue, type, payload, and retry policy; the failed
...
The problem: a customer withdraws an order while its export task is running. Handler execution
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  ...
  <Tab value="Go">
    ...
    func configureExport(
    	...
    	requestedBy := "support@example.com"
    	reason := "customer withdrew the order"
    	if _, err := queue.Cancel(ctx, taskID, workhorse.CancellationRequest{
...
If the task has not started, PostgreSQL cancels it immediately with no invented attempt history. If
it is active, the worker's heartbeat delivers the request. TypeScript aborts `context.signal`,
Python cancels `context.cancellation`, Go cancels the handler's standard context, and Rust reports
it through `context.cancellation()`. A handler that ignores that request loses its lease eventually,
and recovery materializes the cancellation instead of retrying.

## Durable agentic flow

The problem: a model plans work, several tools run independently, and execution must survive a
deploy while waiting for approval. Checkpoint the model call, join tools as child tasks, cross a
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  <Tab value="TypeScript">
    ...
    worker.handle("agent.run", async ({ prompt, conversationId }, context) => {
      const plan = await context.checkpoint("plan", () =>
        model.plan(prompt, { idempotencyKey: `plan:${context.task.id}` }),
      );
  ...
  <Tab value="Python">
    ...
    from workhorse import ChildTaskRequest, EnqueueOptions, HandlerContext, Json, Worker
    ...
    def register_agent(worker: Worker, model: Model, cooldown_ms: int) -> None:
        def run_agent(payload: object, context: HandlerContext) -> dict[str, Json]:
            ...
            plan = context.checkpoint(
                ...
                lambda: model.plan(
                    prompt,
                    idempotency_key=f"plan:{context.task.id}",
                ),
            )
            assert isinstance(plan, dict)
            raw_tools = plan["tools"]
            assert isinstance(raw_tools, list)
            children: list[ChildTaskRequest] = []
    ...
    func registerAgent(worker *workhorse.Worker, model Model, cooldown time.Duration) {
    	worker.Handle("agent.run", func(
    		...
    		planValue, err := handler.Checkpoint("plan", func() (any, error) {
    			return model.Plan(ctx, prompt, "plan:"+handler.Task.ID)
    		})
    		if err != nil {
    			return nil, err
    		}
    		...
    		children := make([]workhorse.ChildTaskRequest, 0, len(rawTools))
    		for _, value := range rawTools {
    			...
    			children = append(children, workhorse.ChildTaskRequest{
    				Name:    toolID,
    				Type:    "agent.tool",
    				Payload: tool,
...
signal, and human-decision boundaries. Rust consumers can run `rust/examples/orchestration.rs` for
child, signal, and human-decision boundaries. Every release lane verifies the public package
imports.
