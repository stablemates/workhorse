...
The synchronous Python package exposes the tracer-bullet form of this API: `Worker.handle`
registers a handler and `Worker.run_once` claims and settles one job. It uses a dedicated
autocommit Psycopg connection so handler code runs after the claim transaction closes. A background
heartbeat renews ownership and delivers cancellation, deadlines, timeouts, and lease loss through
`HandlerContext.cancellation`. Use the TypeScript worker for concurrent slots, notifications, a
continuous loop, and graceful drain while the remaining Python runtime work lands.
