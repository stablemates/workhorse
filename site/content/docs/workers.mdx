...
Tasks sit in PostgreSQL until a process runs them. That process is a `Worker`: a loop that
claims ready tasks, runs your handlers, and records each result. This page covers everything
between `new Worker(queue)` and a clean shutdown.

## Register handlers and run

`Worker.handle` binds a task type to an async function. Registration chains, so one worker can
serve many types. Each handler receives the payload and a `HandlerContext`, and returns a JSON
result that becomes the task's durable outcome.
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  ...
  <Tab value="Go">
    ...
    worker, err := workhorse.NewWorker(pool, workhorse.WorkerOptions{
    	Queues: []string{"email", "billing"}, Concurrency: workerConcurrency,
    })
    if err != nil {
    	return err
    }
...
heartbeat, batch, settlement, telemetry, and drain core. Workers in every SDK language also
...
A worker discovers each queue's [tier](/docs/fast-tier) on its own, so a fast-tier queue needs no
worker option. On the fast tier, a busy TypeScript or Python worker splits its slots into
[cohorts](/docs/fast-tier#workers-need-no-configuration) that complete separately.
...
for. That is normal during a deploy that replaces workers one at a time, where the new release
enqueues a type the old release has never heard of.

The worker hands that claim straight back under its own fence. Workhorse returns the task to its
queue and leaves the attempt count alone, so a worker that does have the handler still gets every
attempt the task was allowed. Failing it instead would spend an attempt on a worker that never ran
anything. A pass that only handed claims back counts as an empty one, so a type no deployed worker
handles is re-checked on the polling cadence rather than continuously.
...
The `HandlerContext` carries the claimed task, its `AbortSignal`, checkpoints, durable waits,
and progress. Every context method writes under the current fence, so handler code cannot
accidentally write as a stale owner after the task moves on.
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  <Tab value="TypeScript">
    ```ts
    const worker = new Worker(queue, {
      queues: ["email", "billing"], concurrency: workerConcurrency, leaseMs: workerLeaseMs,
  ...
  <Tab value="Go">
    ...
    worker, err := workhorse.NewWorker(pool, workhorse.WorkerOptions{
    	Queues: []string{"email", "billing"}, Concurrency: workerConcurrency,
    	LeaseDuration: workerLeaseDuration, WorkerID: "email-worker-1",
    })
    ```
  </Tab>
  <Tab value="Rust">
...
- **`concurrency`** caps simultaneous handlers in this one `Worker`. Replicas multiply that
  capacity. A busy worker refills freed slots with batched claims that overlap in flight, so
  throughput rises with `concurrency` instead of waiting on one claim per task. It never claims
  more tasks than it has free slots. When the fleet must share one budget, use
  [concurrency policies](/docs/concurrency-policies); for a shared start rate, use
  [rate limits](/docs/rate-limits).
- **`leaseMs`** is how long a claim owns a task before the database may recover it. One
  background timer batches every active lease owned by the worker on its configured heartbeat
  cadence. A shorter lease means faster
...
- **`retryDelayMs`** overrides the persisted retry policy — a number, or
  `(attempt, task) => number | undefined`, where `undefined` defers to the database.
...
`WorkerOptions.ScheduleNamespaces` and `WorkerOptions.ScheduleCatchupLimit`. Rust sets
`schedule_namespaces` and `schedule_catchup_limit` on `WorkerOptions`.

Each worker publishes its schedule namespaces with its queue list. The Workers page shows both:
queues control claims, while schedule namespaces control recurring evaluation.
...
`Worker.handleBatch` groups claimed tasks of one type when one provider call can process several
...
The TypeScript `handleBatch`, Python `handle_batch`, Go `HandleBatch`, and Rust `handle_batch` forms
are shown together in [Batch handlers](/docs/batch-handlers), including their positional success and
failure results.
...
The first termination signal stops claims and drains active tasks. A second signal or an expired
...
worker process or prevent later tasks from running.

Rust applications call `run_worker_process`, which drains on `SIGINT` or `SIGTERM`. To own the
boundary instead, pass any shutdown future to `Worker::run`. A handler panic becomes a recorded
attempt failure there too.
...
- **`Admin.setWorkerPaused`** stores an operator pause in `worker_registry`. TypeScript, Python,
  Go, and Rust workers pick it up on their next registration refresh, so a dashboard — or
  `workhorse admin pause-worker` — can pause a process it does not host. A local resume cannot
...
TypeScript configures the cadence with `registryIntervalMs`, Python uses `registry_interval_ms`, Go
uses `RegistryInterval`, and Rust uses `registry_interval`. A registration error does not stop
dispatch, and the worker keeps its last remote-pause decision until a later refresh succeeds.

Each refresh also records which client library the worker runs, at which version, speaking which
protocol. A rolling deploy runs more than one build at once by design, so the dashboard shows this
per worker and `workhorse schema status --json` counts the live workers by the protocol each one
reported. A worker running an SDK older than those fields reports none of them, and the registry
records that as reporting nothing. Producers never register, so the counts are evidence about
workers and not an inventory of everything using the database.

A worker that dies stops refreshing and goes stale in the registry. Automatic maintenance later
removes that row. Its tasks are not lost: their leases expire and
recovery makes them claimable again.

The protocol names make this lifecycle greppable. `claim_many_v1` admits the
tasks that the queue's policies allow and writes each one's `task_runtime` lease; `claim_v1` is the
same call with a limit of one. `heartbeat_many_v1` submits every active lease
and extends the accepted `expires_at` values. `recover_expired_v1` makes abandoned work ready.
Workers claim from `workhorse_tasks`, TypeScript configures fallback polling with `pollMs`, and
...
- [Batch handlers](/docs/batch-handlers) — share one application call across tasks
