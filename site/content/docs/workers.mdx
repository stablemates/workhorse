...
The protocol names make this lifecycle greppable. `claim_v1` writes the `job_runtime` row and its
`expires_at` lease, `heartbeat_v1` extends it, and `recover_expired_v1` makes abandoned work ready.
