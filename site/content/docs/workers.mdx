...
reported. A worker running an SDK older than those fields reports none of them, and that is
recorded as reporting nothing. Producers never register, so the counts are evidence about workers
and not an inventory of everything using the database.
