...
The synchronous Python package exposes this lifecycle through `Worker.handle`, `run`, `run_once`,
`pause`, `resume`, and `stop`. It rotates across configured queues, fills a bounded set of slots,
and drains active handlers before `run` returns. A dedicated autocommit Psycopg connection keeps
handler code outside the claim transaction, while one background heartbeat per slot renews
ownership and delivers cancellation, deadlines, timeouts, and lease loss through
...
Python uses `schedule_namespaces` and `schedule_catchup_limit` for the same boundary.
