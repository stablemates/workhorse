...
autocommit Psycopg connection so handler code runs after the claim transaction closes. Use the
TypeScript worker for concurrent slots, heartbeats, cancellation delivery, notifications, and
graceful drain while the remaining Python runtime work lands.
