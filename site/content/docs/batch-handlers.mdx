...
description: Process several tasks of one type through a shared application call while preserving per-task outcomes.
---

Use TypeScript's `Worker.handleBatch`, Go's `Worker.HandleBatch`, Python's `Worker.handle_batch`, or
Rust's `Worker::handle_batch` when one application call can process several tasks more efficiently.
Each member keeps its own durable identity, lease, fence, retry budget, checkpoints, progress, and
cancellation.
...
Workhorse submits the failure for every member, then each task applies its own retry policy.
...
Rust passes `max_size` and `linger` through `BatchOptions`. Its callback receives `BatchItem`
values, each with a payload and its own `HandlerContext`. It returns one `BatchResult::Succeeded` or
`BatchResult::Failed` per item, in item order.
...
and replay independently inside a shared invocation. Register an ordinary handler when a task needs
those boundaries.
