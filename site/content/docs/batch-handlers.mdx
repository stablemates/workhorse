...
Use TypeScript's `Worker.handleBatch`, or Python's `Worker.handle_batch`, when one application call
can process several jobs more efficiently. Each member keeps its own durable identity, lease,
fence, retry budget, checkpoints, and cancellation.
...
the same `succeeded` and `failed` statuses, and each item carries a `BatchHandlerContext`.
