...
When a `Worker` has a free slot, it calls the `claim_v1` SQL function. PostgreSQL checks any queue
policy, then records the worker ID, a lease expiry, and a fence token on `job_runtime`. The worker
sends heartbeats through `heartbeat_v1` while the handler runs.
...
`job_query` is a separate bounded projection for operator reads. `Admin.listJobs` and the dashboard
