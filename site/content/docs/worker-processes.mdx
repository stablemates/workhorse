...
Your web tier and your task workers have different failure modes, different scaling curves, and
different memory budgets. A dedicated worker process keeps them apart: web replicas serve
traffic, worker replicas process tasks, and the two meet only in PostgreSQL. Workhorse packages
...
every worker. Workers stop asking PostgreSQL for tasks; active handlers keep their leases and
finish. If a claim was already in flight when the signal arrived, it can still commit — the
process now owns that lease, so it drains that task too.
...
them to another worker. Nothing is lost either way — a drained task finished here, an abandoned
one finishes elsewhere.

Two boundaries worth knowing:

- Process shutdown does not abort `HandlerContext.signal` and creates no cancellation evidence.
  When the task itself must stop, call `queue.cancel`.
...
The [Kubernetes deployment reference](/docs/kubernetes) pairs a 110-second process deadline with a
120-second Pod grace, then wires both probe endpoints into a worker Deployment.

## Probes for your orchestrator

The optional probe listener serves exactly two endpoints: liveness (`/livez`) while the process
exists, readiness (`/readyz`) while workers can still ask for tasks. Readiness drops the moment
draining begins, so a rolling deployment routes nothing to a process on its way out.

The probe server is not application ingress. It exposes no task data, no queue data, no metrics,
...
- [Kubernetes](/docs/kubernetes) — deploy schema, worker, and dashboard workloads
