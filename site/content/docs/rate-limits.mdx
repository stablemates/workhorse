...
how much work runs at once, and sixty tasks that each finish in a hundred milliseconds sail through
...
TypeScript applications call `Queue.syncRateLimitPolicies`. `Queue.listRateLimitPolicies` returns
the persisted typed definitions.

Go applications call `Queue.SyncRateLimitPolicies` through their caller-owned executor.
`Queue.ListRateLimitPolicies` returns the persisted typed definitions. Rust applications use
`Queue::sync_rate_limit_policies` and `Queue::list_rate_limit_policies`.

Python applications call `sync_rate_limit_policies` through `Queue` or `AsyncQueue` on their
caller-owned connection. `list_rate_limit_policies` returns the persisted typed definitions.
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  ...
  <Tab value="Python">
    ```python
    queue.sync_rate_limit_policies("workers", [RateLimitPolicyDefinition(
        queue="provider-api",
        rate=RateLimit(limit=60, interval_ms=60_000, burst=10),
        per_key=RateLimit(limit=5, interval_ms=60_000, burst=2),
    )])
    ```
  </Tab>
  <Tab value="Go">
    ```go
    policies, err := queue.SyncRateLimitPolicies(ctx, "workers", []workhorse.RateLimitPolicyDefinition{{
    	Queue:  "provider-api",
    	Rate:   workhorse.RateLimit{Limit: 60, IntervalMS: 60_000, Burst: 10},
    	PerKey: &workhorse.RateLimit{Limit: 5, IntervalMS: 60_000, Burst: 2},
    }})
    ```
  </Tab>
  <Tab value="Rust">
...
minute — and each task start consumes one token. `burst` caps how many tokens survive an idle
...
own refill while another customer's task starts. A keyless task draws from the queue bucket only.
...
Tokens are never refunded. The budget measures starts: a task that succeeds, fails, waits, or loses
its lease keeps its token spent, and every retry start consumes a fresh one. Size the limit for
attempts, not for tasks.
...
active-work cap stays visually distinct from a ready task waiting for its next token.

Python synchronizes policies through `Queue.sync_rate_limit_policies` or
`AsyncQueue.sync_rate_limit_policies`, then reads them through `Queue.list_rate_limit_policies` or
`AsyncQueue.list_rate_limit_policies`. TypeScript exposes current pressure through
`Queue.rateLimitStatuses`.

## One bucket across queues

A rate policy governs one queue. When several queues call the same external service, give them a
named budget with a `rate` through `Queue.syncBudgets`. Every start that names the budget, from any
queue, draws one token from the same bucket, and PostgreSQL refills it from its own clock. A task
names its budget with the `budget` enqueue option. The budget is an extra check beside the queue's
own rate policy and has no per-key bucket. `Queue.budgetStatuses` shows the budget's refilled tokens
and how much sampled ready work waits on it, and the same facts appear in `Queue.health()`. The
[concurrency policies](/docs/concurrency-policies) page shows how a budget is defined in each
language.
