...
- `@workhorse-js/core` provides `Queue`, `Admin`, `Worker`, schema tools, process orchestration, and the CLI.
- `workhorse-pg` provides synchronous and asynchronous Python queue clients and workers.
...
TypeScript applications start with `@workhorse-js/core` plus `pg`. Python and Go applications use
their native SDK with an existing PostgreSQL driver. TypeScript applications add an adapter when
they want to enqueue inside an ORM transaction.
