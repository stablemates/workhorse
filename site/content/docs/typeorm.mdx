---
title: TypeORM
description: Adapt a TypeORM data source and enqueue tasks through transactional entity managers.
---
...
and a committed row with no task is a silent gap. `@stablemates/workhorse-typeorm` closes that gap: it converts
...
```bash
pnpm add @stablemates/workhorse-typeorm typeorm
```
...
manager. The queue sends its SQL through that manager, so the task follows the surrounding commit or
rollback.
...
## Workers use the data source's pool

When the data source initializes, TypeORM's Postgres driver creates a node-postgres pool as
`dataSource.driver.master`. Workers take their heartbeat connection and their `LISTEN/NOTIFY`
connection from it, so initialize the data source before you create a worker. The pool must allow
room for both connections plus claims; TypeORM's `extra.max` sets its size. An integration test
pins that accessor, because TypeORM does not document it.
