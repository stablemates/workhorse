...
Python operator tools use the same split from application enqueueing:

```python
admin = Admin(connection)
async_admin = AsyncAdmin.from_asyncpg(asyncpg_connection)
```

Go applications construct the same operator boundary with `workhorse.NewAdmin(executor)` and call
`ListDeadLetters`, `Redrive`, or `RedriveMany` through their caller-owned pgx or `database/sql`
executor.
...
The Python spelling uses an `AdminAudit` value and returns the same PostgreSQL decision:
...
print(result.status, result.target_job_id)
```
