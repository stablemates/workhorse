...
description: Bound a task by wall-clock time, or bound one attempt's execution, with two independent clocks.
---

Workhorse has two ways to say "this has taken too long", and they answer different questions.
A deadline says the _result_ expires at a wall-clock moment — after that, the task is
...
## A deadline bounds the whole task

`EnqueueOptions.deadline` records an immutable absolute boundary. The clock never stops: it
runs while the task is queued, retrying, sleeping on a durable wait, and executing. Use it when
...
After the deadline passes, PostgreSQL refuses new claims for the task. If a handler is active,
the next heartbeat returns `deadline_exceeded` and the worker aborts `ctx.signal` with
`DeadlineExceededError`. Either way the task ends terminally — a deadline never schedules
another retry, because retrying expired work only delays admitting it failed.

Redriving a dead-lettered task clears the old deadline. Copying an expired business boundary
would make the new task fail on arrival.
...
[durable wait](/docs/durable-execution), the task holds no worker slot and the accounting
...
`deadline` with `execution_timeout_ms` in Python or Rust, or `Deadline` with `ExecutionTimeoutMS` in
Go.

Together they bound each attempt by its execution budget and abandon the whole task at the
