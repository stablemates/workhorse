...
pauses — a long sleep uses none of its execution budget. When the budget
...
exhausted budget from that job's `heartbeat_many_v1` result, then aborts the handler's signal — the same
