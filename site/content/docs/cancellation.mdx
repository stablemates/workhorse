...
cancellation in Workhorse follows from that fact. A task that is not running can be finished on
the spot; a task whose handler is executing can only be asked to stop, and your handler decides
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  ...
  <Tab value="Go">
    ...
    requestedBy := actor.Email
    reason := "customer withdrew the order"
    result, err := queue.Cancel(ctx, taskID, workhorse.CancellationRequest{
...
- **`ready`, `scheduled`, or a [durable wait](/docs/durable-execution)** — no worker holds
  the task, so PostgreSQL writes the canceled outcome immediately. A task that never started
...
- If a handler ignores the signal, nothing stops it. The task stays active until its lease
...
- Cancel first — Workhorse refuses the later completion; it cannot resurrect the task.
...
Canceling a task created by a [schedule](/docs/schedules) affects that occurrence only. The
schedule definition stays enabled until deployment synchronization changes it.

All clients reach this database-owned transition through `cancel_v1`, even when their public method
names follow different language conventions.
