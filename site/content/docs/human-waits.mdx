...
The dashboard lists pending decisions with their job identity, name, stored context, and effective
deadline. Its server replaces browser attribution with the authenticated principal before calling
the queue mutation.
...
An application can put `dashboard.quickAction` in the stored context with a button `label` and JSON
`result`. The dashboard then offers that result without asking the operator to type JSON. It never
invents a quick action for generic decisions.
