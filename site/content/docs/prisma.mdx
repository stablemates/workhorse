---
title: Prisma
description: Adapt a Prisma client and enqueue tasks inside caller-owned interactive transactions.
---
...
and a committed row with no task is a silent gap. `@stablemates/workhorse-prisma` closes that gap: it converts
...
client. The queue sends its SQL through that client, so the task follows the surrounding commit or
rollback.
...
Prisma's engine has no node-postgres pool. A process that runs workers passes a separate
caller-owned pool as `pool`. Workers take their heartbeat connection and their `LISTEN/NOTIFY`
connection from it, so busy handlers cannot starve lease renewal. Without it, a worker refuses to
start unless you set `sharedHeartbeats`. It then sends heartbeats through Prisma and dispatches by
bounded polling.
