...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  ...
  <Tab value="Go">
    ...
    tools, err := handler.RunChildrenAll(toolRequests)
...
Python and Rust handlers use `set_progress` and `get_progress`; Go handlers use `SetProgress`
and `GetProgress`. Every surface write the same fenced projection, so the dashboard sees one
...
- [Child tasks](/docs/child-tasks) — delegate and join tool work
