...
commands only read. To answer a wait, use `admin signal <job-id>` or
`admin complete-human <job-id>` with `--name`, an explicit `--request-id`, and `--actor`.
...
To continue `admin jobs`, `admin timeline`, or `admin failures`, pass the returned `nextCursor`
object through `--cursor` with the same filters. Jobs accept `--created-after` and `--created-before`;
