...
Guarded commands — `admin cancel`, `admin redrive`, `admin pause`, `admin resume`,
`admin purge` — refuse to run until the target database is named explicitly with `--env`,
verified against `current_database()` on the live connection, and the operation is confirmed
interactively or with `--yes`. Attribution, redrive idempotency, and purge idempotency follow the
same contracts as the programmatic APIs. `admin purge` deletes one queue's waiting jobs, leaves
running ones alone, and reports the deleted count as `deletedCount` under `--json`.
