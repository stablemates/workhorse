...
Every summary boundary is a UTC boundary, whatever timezone the database is set to, so a day
summary covers a UTC day and names the same day for every operator reading the chart.
...
window is correct the instant a task runs; a lagging rollup means a longer live section and a
...
TypeScript, Python, Go, and Rust queues store the active W3C trace context when they enqueue a task.
Every worker restores that context before creating its handler span, so one trace can cross
processes and languages. The workers also emit the same span and metric names, so one set of
dashboards can cover a mixed-language fleet.
...
inert and worker behavior is unchanged. The Rust crate always emits `tracing` spans, and its
`opentelemetry` feature adds metrics and trace propagation.
...
process startup. Importing core or the adapter has no registration side effect, and Workhorse
rejects a competing active provider. Keep the returned cleanup as `unregisterTelemetry`, then call
`unregisterTelemetry()` after workers and observers stop.

Worker metrics use queue, task type, and a closed outcome vocabulary. Task IDs remain on sampled
...
Pause is cooperative: the worker notices on its next registry refresh, stops asking for tasks, and
...
[Cancellation](/docs/cancellation) owns durable requests to stop one task.
[Dead letters and redrive](/docs/dead-letters) owns replaying tasks that failed terminally.
...
tasks, queues, schedules, failures, workers, and maintenance state — as aligned tables for
...
A stalled durable handler is inspectable from the terminal too. `admin checkpoints <task-id>` and
`admin waits <task-id>` list one task's restart boundaries and durable timer waits, and `--name`
...
commands only read. To answer a wait, use `admin signal <task-id>` or
`admin complete-human <task-id>` with `--name`, an explicit `--request-id`, and `--actor`.
Supply exactly one of `--payload-json` or `--payload-file`. Repeating the same identity, actor,
and payload returns a successful duplicate; a conflicting answer or unavailable wait fails.

Guarded commands — `admin cancel`, `admin redrive`, `admin pause`, `admin resume`, `admin purge`,
`admin pause-worker`, `admin resume-worker`, `admin redrive-many`, `admin signal`, and
`admin complete-human` — refuse to run until the target database is named
...
waiting tasks, leaves running ones alone, and reports the deleted count as `deletedCount` under
...
To continue `admin tasks`, `admin timeline`, or `admin failures`, pass the returned `nextCursor`
object through `--cursor` with the same filters. Tasks accept `--created-after` and `--created-before`;
failures accept `--finished-after`, `--finished-before`, repeated `--tag`, and `--error-name`.
Time filters require a timezone and include the lower bound while excluding the upper bound.

`admin redrive-many` recovers a bounded, oldest-first page using those failure filters.
Preview with `--dry-run --reason "provider restored"` without environment confirmation or writes.
Execute with an explicit `--request-id`, `--env`, and confirmation. Reuse the request identity on retries
and continue with recovery's own `nextCursor`; failure-list cursors have the opposite ordering.
A preview does not reserve its candidates.
...
`workhorse.tasks.enqueued`, `workhorse.tasks.claimed`, `workhorse.queue.paused`,
`workhorse.tasks.enqueue.outcomes`, `workhorse.handler.executions`, `workhorse.handler.duration`,
`workhorse.tasks.completed`, `workhorse.tasks.failed`, and `workhorse.tasks.retried`. The
