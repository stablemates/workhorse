...
TypeScript and Python workers emit the same worker span, metric, and structured event names, so one
set of dashboards can cover a mixed-language fleet. The Python package exposes this through its
optional `telemetry` extra, which installs the API without choosing an SDK or exporter. If the host
does not configure an SDK, telemetry stays inert and worker behavior is unchanged.
