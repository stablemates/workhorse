...
2. **Database connectivity:** An enqueue producer needs a supported Workhorse client and a
   PostgreSQL session. Node.js functions can use the `Pool` exported by `@stablemates/workhorse`. An edge
...
to `Queue.enqueue`. The task then commits or rolls back with the business write.
...
[Cloud Run worker pool](https://docs.cloud.google.com/run/docs/overview/what-is-cloud-run#services-jobs-worker-pools). Give that worker a pool with room for its
dedicated connections; [connection poolers](/docs/pgbouncer) explains the budget and the opt-out for
a pool that cannot spare them.
...
- [Transactional enqueue](/docs/enqueue) — commit the task with the business write
