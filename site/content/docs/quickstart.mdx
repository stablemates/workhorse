---
title: Quickstart
description: Install the schema, run your first task, then kill the worker mid-task and watch it finish anyway.
---

In five minutes you will enqueue a task, run it, and then prove the interesting part: a task that
survives its own worker being killed. Choose TypeScript, Python, Go, or Rust for the application
code.
...
Workhorse lives entirely inside your database: tables for tasks and versioned SQL functions for
...
A TypeScript project runs the binary it already depends on:

```bash
npm exec --no -- workhorse schema install
```

A Python, Go, or Rust project has no `node_modules` to resolve that binary from, so it names the
version instead — the same version as the SDK it installed above:

```bash
npx --package @stablemates/workhorse@0.3.0 workhorse schema install
```

That step runs on Node.js, even when the application is written in Python, Go, or
Rust.
[Installation](/docs/installation) explains why the two forms differ, and
...
<Tabs items={["TypeScript", "Python", "Go", "Rust"]}>
  <Tab value="TypeScript">
    ...
    const taskId = await queue.enqueue("email.welcome", { to: "ada@example.com" });

    await worker.runOnce(); // one claim-and-run pass; production uses worker.run()
    console.log(await admin.getTask(taskId));
    await pool.end();
    ```
  ...
  <Tab value="Python">
    ...
    with psycopg.connect(database_url) as connection:
        task_id = Queue(connection).enqueue("email.welcome", {"to": "ada@example.com"})
  ...
  <Tab value="Go">
    ...
    func run() error {
    	...
    	taskID, err := queue.Enqueue(ctx, "email.welcome", map[string]any{
    		"to": "ada@example.com",
...
ship `Admin` clients for the same operator reads. The Rust example reads the outcome through its
`Admin` client.

## 4. Kill the worker. The task finishes anyway.
...
const worker = new Worker(queue, { leaseMs: 5_000 }).handle(
  ...
  async (payload: { orderId: string }, ctx) => {
    ...
    if (!process.env.SURVIVED) {
      ...
      process.exit(1); // the worker dies mid-task
    }
...
// The idempotency key makes the second run return the same task
...
SURVIVED=1 node crash-demo.ts  # finishes the task
```
...
The second run picks the task up once the expired lease is recovered — within a few seconds — and
...
argument and the task commits — or rolls back — with your business write.
