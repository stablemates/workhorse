...
task. Changing the name, payload, type, options, or set membership raises `ChildConflictError`, so a
...
Python handlers call `run_child` or `run_children`. Go handlers call `RunChild` or
`RunChildren`. Rust handlers call `run_child` or `run_children` with `ChildTaskRequest` values. Every
set method retains the declared names. Use `runChildrenAll`, `run_children_all`, or `RunChildrenAll`
when a child failure or cancellation must propagate to the parent.

Go represents the three tags as `ChildSucceeded`, `ChildFailed`, and `ChildCanceled`. Rust
represents them as the `ChildOutcome` variants `Succeeded`, `Failed`, and `Canceled`.
...
`Admin.getTask` and `Admin.listTasks` expose `parentTaskId` and `childTaskIds`.
`Admin.getChildLineage(taskId)` reads retained edges in either direction, and the dashboard links
...
- [Task dependencies](/docs/task-dependencies) — block work declared by a producer
