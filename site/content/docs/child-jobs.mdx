...
`CreateChildren`. Both set methods retain the declared names and request order.
...
`ChildResult`. Use `HandlerContext.checkpoint` around replay-sensitive surrounding work. Workhorse
