...
Go handlers call `CreateChild` or `CreateChildren`. The set method returns named `ChildResult`
values in request order, so Go callers do not depend on map iteration to preserve the fan-out order.
