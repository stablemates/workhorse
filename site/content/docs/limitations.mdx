...
## Python and Go have a smaller operator API

Python and Go ship transactional queue clients and complete worker runtimes over the shared SQL
protocol. They do not yet ship the public `Admin` client available in TypeScript, so applications
use the embedded or standalone dashboard for operator reads and controls.
