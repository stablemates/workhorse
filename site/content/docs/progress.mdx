...
one, `Admin.getProgress` reads it by task ID, and a custom runtime can write through
`Queue.updateProgress` with the claimed task and worker identity. The
...
as the TypeScript record. Rust's `set_progress` and `get_progress` read and write the value alone;
`Admin::get_progress` returns the full record.

## Why progress is rate-limited

Every changed value updates one `task_progress` row and appends a small `progress_updated`
...
task identity — so a failed task's last reported phase remains readable evidence.
...
- [Queries and timelines](/docs/queries) — read task and lifecycle state
