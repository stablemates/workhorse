...
- `assertSchemaCompatible(database)` fails startup on a runtime/schema mismatch, throwing
  `SchemaCompatibilityError` with the `code` Python, Go, and Rust also report;
...
From a deployment step outside such a project — a Python, Go, or Rust pipeline — name the package
