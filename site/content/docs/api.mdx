...
provides operator reads and controls, and `Worker` runs tasks. A handful of schema and process helpers surround them. This page
...
**Producing work.** `enqueue(type, payload, options?, transaction?)` accepts one task and returns
its ID; pass an open transaction as the last argument to commit the task with your data.
`enqueueMany(requests, transaction?)` accepts a bounded batch. `enqueueWithResult` and
`enqueueManyWithResults` return each retained task ID with an `EnqueueOutcome` describing whether
...
**Stopping work.** `cancel(taskId, { requestedBy, reason })` records an attributed cooperative
...
methods under a claimed task's fence; application code normally reaches those through
`HandlerContext`.
...
**Reading evidence.** `getTask`, `listTasks`, and `getTaskTimeline` inspect tasks.
...
`schedules` reads recurring definitions, and `runTaskNow` releases one scheduled task early.

## Worker

`new Worker(queue, options?)` claims and runs tasks with bounded concurrency.

- `handle(type, handler)` registers a typed handler and returns the worker for chaining.
- `handleBatch(type, options, handler)` groups tasks of one type while preserving a separate outcome
...
`maintenanceRoutinePollMs`, `registryIntervalMs`), schedule evaluation (`scheduleNamespaces`,
...
- `task` — the `ClaimedTask` with ID, attempt, and metadata.
...
- `CheckpointConflictError` and `WaitConflictError` — a durable name was reused with different
  content; `WaitLimitExceededError` bounds waits per task.
...
- `TaskContractValidationError`, `TaskContractUnavailableError`, and `TaskValueSizeLimitError` —
...
- `assertSchemaCompatible(database)` fails startup on a runtime/schema mismatch, throwing
  `SchemaCompatibilityError` with the `code` Python and Go also report;
...
- `schema contract --yes` applies the pending contract step, retiring the protocols it names.
