...
Install the SDK beside the PostgreSQL driver your application already uses.
...
<Tabs items={["TypeScript", "Python", "Go"]}>
  ...
  <Tab value="Python">
    ...
    Use the `asyncpg` extra when that is your application's driver.
...
```bash
npx --package @workhorse-js/core workhorse schema install
npx --package @workhorse-js/core workhorse schema migrate
npx --package @workhorse-js/core workhorse schema status
npx --package @workhorse-js/core workhorse schema status --json
```
