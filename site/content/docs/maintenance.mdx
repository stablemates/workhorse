...
TypeScript, Python, and Go workers all call `run_maintenance_v1`, which keeps the slow phase order
in PostgreSQL. A deployment therefore needs no TypeScript worker to replenish partitions, roll up
statistics, retire history, or prune terminal and worker records.
