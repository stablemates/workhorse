...
all. Workers in TypeScript, Python, Go, and Rust claim and run those tasks through one SQL protocol,
and the database keeps the evidence: every attempt, checkpoint, wait, and outcome is a row an
operator can query.
...
Four lines ship from the same repository and speak the same protocol:
...
- The Rust crate `workhorse` on crates.io.
