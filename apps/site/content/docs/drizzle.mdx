...
The adapter does not close caller-owned resources unless you supply `close`. Add that hook only when
the adapter owns the pool, because `adapter.close` may run during process shutdown or failed startup.
