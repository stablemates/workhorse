...
platform. Workhorse has no workflow or DAG engine, no job priorities, and no exactly-once external
effects — external calls still need provider idempotency keys. If you need a queue that scales
