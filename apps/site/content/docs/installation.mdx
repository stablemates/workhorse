...
Add a database adapter (`@workhorse/drizzle`, `@workhorse/prisma`, `@workhorse/typeorm`,
`@workhorse/kysely`) or `@workhorse/dashboard` only when that integration belongs in the same
process.
...
unversioned, or older is refused. It will not guess how to migrate durable state between protocol
versions — treating installation as an upgrade path could corrupt live jobs.
