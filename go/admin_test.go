...
func TestAdminRoutesEveryOperatorOperationThroughThePublicClient(t *testing.T) {
	...
	operation := func(rows ...workhorse.Row) {
		responses = append(responses, []workhorse.Row{{"kind": "schema", "version": int64(testSchemaVersion)}, {"kind": "protocol", "version": int64(workhorse.ProtocolVersion)}}, rows)
	}
	operation() // ListTasks
	operation() // GetTask
	operation() // GetTaskTimeline
	operation() // ListDeadLetters
	operation(workhorse.Row{"status": "created", "source_task_id": "source", "target_task_id": "target", "source_state": "failed", "target_state": "ready", "requested_at": now})
	...
	if _, err := admin.ListTasks(ctx, workhorse.TaskListQuery{}); err != nil {
		t.Fatal(err)
	}
	if _, err := admin.GetTask(ctx, "00000000-0000-4000-8000-000000000001"); err != nil {
		t.Fatal(err)
	}
	if _, err := admin.GetTaskTimeline(ctx, "00000000-0000-4000-8000-000000000001", workhorse.TaskTimelineQuery{}); err != nil {
		t.Fatal(err)
	}
	if _, err := admin.ListDeadLetters(ctx, workhorse.DeadLetterQuery{}); err != nil {
		t.Fatal(err)
	}
	if result, err := admin.Redrive(ctx, "00000000-0000-4000-8000-000000000001", audit); err != nil || result.TargetTaskID == nil {
	...
	for _, function := range []string{"list_tasks_v1", "list_task_timeline_v1", "list_dead_letters_v1", "redrive_v1", "redrive_many_v1", "set_queue_paused_v1", "purge_queue_v1", "set_worker_paused_v1"} {
...
func TestAdminMeasuresActorAndReasonInCharacters(t *testing.T) {
	executor := &queueExecutor{responses: [][]workhorse.Row{
		{{"kind": "schema", "version": int64(testSchemaVersion)}, {"kind": "protocol", "version": int64(workhorse.ProtocolVersion)}},
