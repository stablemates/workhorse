...
const (
	...
	contractTaskTypeJSONField           = "taskType"
...
const (
	...
	syncScheduleDefinitionsStatementName = "sync_schedule_definitions_v2"
	...
	releaseOwnedStatementName            = "release_owned_v1"
	...
	taskBlockedValue           = "blocked"
	taskScheduledValue         = "scheduled"
	taskReadyValue             = "ready"
	taskActiveValue            = "active"
	taskSucceededValue         = "succeeded"
	taskFailedValue            = "failed"
	taskCanceledValue          = "canceled"
	...
	workerReleaseReleased                          = "released"
	workerReleaseNotDue                            = "not_due"
	...
	taskNotificationChannel                        = "workhorse_tasks"
	taskNotificationWildcard                       = "*"
	listenForTasksStatement                        = "LISTEN workhorse_tasks"
	unlistenForTasksStatement                      = "UNLISTEN workhorse_tasks"
	notificationListenerErrorKey                   = "error"
	redactedHandlerErrorNameValue                  = "RedactedTaskError"
	redactedHandlerErrorTextValue                  = "Task handler failed; details redacted"
	...
	scheduleCatchupSkipValue            = "skip"
	scheduleCatchupLatestValue          = "latest"
	scheduleCatchupAllValue             = "all"
	...
	uniqueDependenciesMessage           = "%w: dependencies must contain unique prerequisite task IDs"
	dependencyCountMessage              = "%w: dependencies accepts at most %d prerequisite task IDs"
	idempotencyConflictMessage          = "PostgreSQL rejected a materially different idempotent enqueue"
	dependencyCycleMessage              = "PostgreSQL rejected a cyclic task dependency"
	dependencyLimitExceededMessage      = "PostgreSQL rejected a task dependency limit"
	...
	fastTierCheckpointsFeature          = "checkpoints"
	fastTierDurableWaitsFeature         = "durable waits"
	fastTierSignalWaitsFeature          = "signal waits"
	fastTierHumanWaitsFeature           = "human waits"
	fastTierChildTasksFeature           = "child tasks"
	completeManyAndClaimStatementName   = "complete_many_and_claim_v1"
	...
	staleLeaseErrorFormat               = "%s for task %s"
	...
	workerCohortsRangeMessage           = "worker cohorts must be between 1 and the concurrency"
	...
	shutdownIncompleteMessage           = "worker shutdown grace period expired while handlers were still running"
	workerShutdownIncompleteFormat      = "%w: %d abandoned"
	workerRegistryRangeMessage          = "worker registry interval must be at least 100 whole milliseconds"
	workerRoutineIntervalRangeMessage   = "worker maintenance routine interval must be a positive whole number of milliseconds"
	...
	leaseLostMessage                    = "task lease was lost"
	leaseLostErrorMessage               = "Task lease was lost"
	cancellationRequestedMessage        = "task cancellation was requested"
	deadlineExceededMessage             = "task deadline was exceeded"
	executionTimeoutMessage             = "task execution timeout was exceeded"
	taskLifecycleErrorFormat            = "%s for task %s"
	attemptLifecycleErrorFormat         = "%s for task %s attempt %d"
	...
	batchOutcomeCountFormat             = "batch handler for %s returned %d outcomes for %d tasks"
	...
	invalidReleaseResultMessage         = "PostgreSQL returned an invalid release result"
	rejectedReleaseStatusFormat         = "PostgreSQL rejected the release with status %s"
	durableWaitSuspensionMessage        = "durable wait suspended the handler"
	checkpointLeaseLostErrorFormat      = "cannot save checkpoint %s for task %s because the lease is stale or expired"
	checkpointConflictErrorFormat       = "checkpoint %s for task %s already exists with a different value"
	waitLeaseLostErrorFormat            = "cannot schedule wait %s for task %s because the lease is stale or expired"
	...
	progressLeaseLostErrorFormat        = "cannot update progress for task %s because the lease is stale or expired"
	progressRateLimitErrorFormat        = "cannot update progress for task %s yet; retry after %s"
	...
	childLeaseLostErrorFormat           = "cannot create a child for task %s because the lease is stale or expired"
	childConflictErrorFormat            = "child %s for task %s already exists with a different request"
	childLimitExceededErrorFormat       = "task %s exceeds the supported child limit"
	childResultLimitExceededErrorFormat = "joined child results for task %s exceed its configured size limit"
	tooManyChildOptionsMessage          = "%w: create child accepts at most one EnqueueOptions value"
	childKeyedOptionsMessage            = "%w: child tasks cannot use coalescing or dependency enqueue options"
	...
	telemetryReleasedValue       = "released"
	telemetrySuspendedValue      = "suspended"
	telemetryTimeoutValue        = "timeout"
	telemetryClaimedValue        = "claimed"
	telemetryEmptyValue          = "empty"

	tasksClaimedInstrument             = "workhorse.tasks.claimed"
	tasksCompletedInstrument           = "workhorse.tasks.completed"
	tasksFailedInstrument              = "workhorse.tasks.failed"
	tasksRetriedInstrument             = "workhorse.tasks.retried"
	...
	tasksClaimedDescription            = "Tasks claimed for handler execution"
	tasksCompletedDescription          = "Tasks completed under a valid lease"
	tasksFailedDescription             = "Handler failures submitted to PostgreSQL"
	tasksRetriedDescription            = "Failed or expired tasks returned to live work"
	...
	handlerBatchSizeDescription        = "Tasks delivered in one batch handler invocation"
	...
	taskIDAttribute                = "workhorse.task.id"
	...
	releaseStatusAttribute         = "workhorse.release.status"
	...
	taskClaimedEvent               = "workhorse.task.claimed"
	handlerStartedEvent            = "workhorse.handler.started"
	executionFinishedEvent         = "workhorse.task.execution_finished"
	...
	handlerMissingEvent            = "workhorse.handler.missing"
	taskReleaseProcessedEvent      = "workhorse.task.release_processed"
	...
	handlerStartedLogMessage       = "Task handler started"
	executionFinishedLogMessage    = "Task execution finished"
	handlerFinishedLogMessage      = "Task handler finished"
	taskCompletedLogMessage        = "Task completed"
	taskFailureProcessedLogMessage = "Task attempt failure processed"
	handlerMissingLogMessage       = "No handler registered for the claimed task type"
	taskReleaseProcessedLogMessage = "Owned task release processed"
...
const (
	...
	signalWaitLeaseLostErrorFormat      = "signal wait %s for task %s cannot be recorded because the lease is stale or expired"
	signalWaitConflictErrorFormat       = "signal wait %s for task %s is already waiting for delivery"
	signalWaitLimitExceededErrorFormat  = "task %s already has the maximum number of signal waits"
	signalIdempotencyConflictFormat     = "signal %s for task %s received a different request for a retained idempotency key"
	...
	humanWaitLeaseLostErrorFormat       = "human wait %s for task %s cannot be recorded because the lease is stale or expired"
	humanWaitAlreadyWaitingErrorFormat  = "human wait %s for task %s is already waiting for completion"
	humanWaitLimitExceededErrorFormat   = "task %s already has the maximum number of human waits"
