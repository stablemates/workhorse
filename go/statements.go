...
var adminStatementRegistry = map[string]string{
	"list_jobs":         `SELECT listed.job_id::text job_id,listed.queue_name,listed.job_type,listed.concurrency_key,listed.priority,listed.tags,listed.state,dependency.prerequisite_job_id,dependency.prerequisite_job_ids,CASE WHEN listed.state='blocked' THEN 'prerequisite_pending' END blocked_reason,parent_edge.parent_job_id::text parent_job_id,children.child_job_ids,listed.current_attempt,listed.max_attempts,listed.retry_policy,listed.deadline_at,listed.execution_timeout_ms::text execution_timeout_ms,listed.run_at,listed.cancel_requested_at,listed.cancel_requested_by,listed.cancel_reason,listed.created_at,listed.updated_at,listed.payload,listed.payload_status,listed.payload_bytes,listed.has_more,listed.cursor_created_at::text cursor_created_at,listed.cursor_signature FROM workhorse.list_jobs_v1($1::jsonb,$2::integer,$3::timestamptz,$4::uuid,$5::text,$6::jsonb) listed LEFT JOIN LATERAL(SELECT CASE WHEN count(*)=1 THEN(array_agg(prerequisite_job_id::text))[1]END prerequisite_job_id,COALESCE(array_agg(prerequisite_job_id::text ORDER BY prerequisite_job_id)FILTER(WHERE prerequisite_job_id IS NOT NULL),'{}')prerequisite_job_ids FROM workhorse.job_dependency WHERE dependent_job_id=listed.job_id)dependency ON true LEFT JOIN workhorse.job_child parent_edge ON parent_edge.child_job_id=listed.job_id LEFT JOIN LATERAL(SELECT COALESCE(array_agg(child_job_id::text ORDER BY child_job_id)FILTER(WHERE child_job_id IS NOT NULL),'{}')child_job_ids FROM workhorse.job_child WHERE parent_job_id=listed.job_id)children ON true`,
	"get_job":           `SELECT j.id::text id,j.queue_name,j.job_type,j.concurrency_key,j.priority,workhorse.redact_top_level_keys_v1(j.payload,j.payload_redact_keys)payload,j.contract_version,j.tags,j.retry_policy,j.deadline_at,j.execution_timeout_ms::text execution_timeout_ms,COALESCE(r.state,o.state)state,dependency.prerequisite_job_id,dependency.prerequisite_job_ids,CASE WHEN r.state='blocked'THEN'prerequisite_pending'END blocked_reason,parent_edge.parent_job_id::text parent_job_id,children.child_job_ids,COALESCE(r.current_attempt,o.current_attempt)current_attempt,j.max_attempts,COALESCE(r.fence_token,o.fence_token)::text version,COALESCE(r.run_at,o.run_at)run_at,workhorse.redact_top_level_keys_v1(o.result,j.result_redact_keys)result,COALESCE(r.error,o.error)error,r.cancel_requested_at,r.cancel_requested_by,r.cancel_reason,p.progress_value,p.revision::text progress_revision,p.attempt progress_attempt,p.fence_token::text progress_fence_token,p.worker_id progress_worker_id,p.created_at progress_created_at,p.updated_at progress_updated_at,j.created_at,COALESCE(r.updated_at,o.updated_at)updated_at FROM workhorse.job j LEFT JOIN workhorse.job_runtime r ON r.job_id=j.id LEFT JOIN workhorse.job_outcome o ON o.job_id=j.id LEFT JOIN LATERAL(SELECT CASE WHEN count(*)=1 THEN(array_agg(prerequisite_job_id::text))[1]END prerequisite_job_id,COALESCE(array_agg(prerequisite_job_id::text ORDER BY prerequisite_job_id)FILTER(WHERE prerequisite_job_id IS NOT NULL),'{}')prerequisite_job_ids FROM workhorse.job_dependency WHERE dependent_job_id=j.id)dependency ON true LEFT JOIN workhorse.job_child parent_edge ON parent_edge.child_job_id=j.id LEFT JOIN LATERAL(SELECT COALESCE(array_agg(child_job_id::text ORDER BY child_job_id)FILTER(WHERE child_job_id IS NOT NULL),'{}')child_job_ids FROM workhorse.job_child WHERE parent_job_id=j.id)children ON true LEFT JOIN workhorse.job_progress p ON p.job_id=j.id WHERE j.id=$1::uuid`,
	...
	"list_signal_waits": `SELECT job_id::text job_id,queue_name,job_type,signal_name wait_name,NULL::jsonb context,attempt,created_at,deadline_at,created_at::text cursor_created_at FROM workhorse.dashboard_signal_wait_v1 WHERE($2::timestamptz IS NULL OR(created_at,job_id,signal_name)>($2::timestamptz,$3::uuid,$4::text))ORDER BY created_at,job_id,signal_name LIMIT $1::integer`,
	"list_human_waits":  `SELECT job_id::text job_id,queue_name,job_type,token_name wait_name,context,attempt,created_at,deadline_at,created_at::text cursor_created_at FROM workhorse.dashboard_human_wait_v1 WHERE($2::timestamptz IS NULL OR(created_at,job_id,token_name)>($2::timestamptz,$3::uuid,$4::text))ORDER BY created_at,job_id,token_name LIMIT $1::integer`,
	...
	"set_worker_paused": `SELECT worker_id,paused,paused_at,paused_by,paused_reason FROM workhorse.set_worker_paused_v1($1::text,$2::boolean,$3::text,$4::text,$5::text)`,
}
