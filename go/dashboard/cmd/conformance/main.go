	...
	harness := func(executor workhorse.Executor, readOnly bool) http.Handler {
		...
		if !readOnly {
			...
			procedures["setSchedulePaused"] = func(ctx context.Context, input any, actor string) (any, error) {
				value := input.(map[string]any)
				rows, err := executor.Query(ctx, `SELECT workhorse.set_schedule_paused_v1($1::text,$2::text,$3::boolean,$4::text,$5::text) AS paused`, value["namespace"], value["name"], value["paused"], actor, "Dashboard operator request")
				if err != nil {
					return nil, err
				}
				if len(rows) == 0 {
