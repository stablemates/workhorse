...
This document records the problem, the design, the schema, the read path, the operational contract, and the limits of the implementation retained in schema version 23.
...
- **Schema installation, not migration.** This remains part of schema version 23 under the pre-release policy in [`schema-lifecycle.md`](schema-lifecycle.md): the canonical schema changed in place, and an existing database is reinstalled rather than upgraded.
