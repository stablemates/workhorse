...
This document records the problem, the design, the schema, the read path, the operational contract, and the limits of the implementation retained in schema version 24.
...
- **Schema baseline.** Rolling statistics entered the version 23 baseline described in [`schema-lifecycle.md`](schema-lifecycle.md). The version 24 step leaves these relations unchanged.
