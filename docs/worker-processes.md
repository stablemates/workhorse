...
- `Admin.setWorkerPaused(workerId, paused, { actor, reason, requestId })` commits the request to
...
4. Continues the worker's batched heartbeat loop and waits for active handlers to settle.
...
A worker with `concurrency: 8` uses one dispatch coordinator and one batched claim query to fill free slots. PostgreSQL checks each member serially inside that call. One heartbeat batch covers up to eight handler executions.
...
Use one multi-slot `Worker` for queues that share a handler registry and operational policy. Add more worker definitions only when they require materially different:
