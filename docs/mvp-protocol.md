...
This is the compact schema version 1 protocol reference. The clean-install schema stores bounded
...
| --------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `job`                 | Stable identity and current accepted definition                            | Insert at acceptance; replace pending definition through keyed debounce               |
...
| `job_query`           | Bounded operator routing projection                                        | Insert with identity; update only for a pending debounce replacement                  |
...
| `maintenance_state`   | Tick execution, per-task due state, and retained-history safety watermark  | SQL-owned maintenance state                                                           |
...
12. `list_dead_letters_v1` validates failure-only filters and returns a bounded descending cursor page from immutable outcomes with priority, persisted top-level payload redaction, and redrive counts.
...
14. `list_jobs_v1` validates exact queue/type/state/creation-time filters, a bounded payload projection, and a filter-bound immutable cursor. It scans `job_query` in immutable creation order, joins authoritative runtime or outcome state, and returns at most 1,000 candidates before joining `job` for priority and optional top-level-redacted payload output. Persisted contract keys are always combined with caller projection keys.
15. `list_job_timeline_v1` merges retained `job_event` and `attempt_history` rows into one latest-first bounded cursor stream with the accepted job priority, frozen before any attempt starts. Unknown identities and identities whose history was fully retired both return an empty stream; use point lookup when that distinction matters.
...
`HandlerContext.waitForSignal(name, { timeoutMs })` creates a separate named external-delivery boundary. A first
...
The optional timeout accepts 1 through 604,800,000 milliseconds and can shorten the seven-day
default; an earlier job deadline wins, and expiry terminally fails instead of resuming without a value.
...
`HandlerContext.waitForHuman(name, context, { timeoutMs })` uses the same timeout contract for a
named human decision. PostgreSQL retains bounded context, releases the lease, and terminally fails
an unanswered decision rather than replaying handler code without an operator result.
...
- Worker tick passes run at most once per `maintenanceIntervalMs` (default one second). Workers offer schedule namespaces on the same cadence and poll the three slow tasks at most once per `maintenanceTaskPollMs` (default 60 seconds), while PostgreSQL owns their database-global due state. `fire_due_schedules_v1` takes one transaction advisory lock per namespace, and `fire_schedule_v1` retains occurrence deduplication. Workers with the same namespace cannot duplicate work, while different namespace sets progress independently.
