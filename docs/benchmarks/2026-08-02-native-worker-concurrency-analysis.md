...
- [`results/2026-08-02-competitor-native-worker-smoke.json`](results/2026-08-02-competitor-native-worker-smoke.json)
...
## External competitor smoke run

This smoke profile used 30 preloaded no-op jobs, three repetitions, and native per-job handlers.

| Product                 | Concurrency 1 | Concurrency 2 |
| ----------------------- | ------------: | ------------: |
| Workhorse native Worker |    558 jobs/s |    702 jobs/s |
| Graphile Worker         |    742 jobs/s |  1,273 jobs/s |
| pg-boss, `batchSize: 1` |   2.14 jobs/s |   4.28 jobs/s |

The safe interpretation is:

- Workhorse's native Worker is competitive for a small local run but scales less strongly than Graphile on
  this no-op workload because Workhorse fills slots through serial per-job claims;
- the topology scenario independently confirms that concurrent claim loops improve no-op throughput at the
  cost of higher database pressure;
- pg-boss's per-job result primarily exposes its configured polling cadence and must not be interpreted as
  maximum pg-boss capacity. Its batch mode is a different handler contract and remains a separately labeled
  sensitivity.
