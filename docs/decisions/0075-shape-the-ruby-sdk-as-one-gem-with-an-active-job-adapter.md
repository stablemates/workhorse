# ADR 0075: Shape the Ruby SDK as one gem with a partial Active Job adapter
...
Three facts shape the design more than the SDK itself does.
...
The gem has two runtime dependencies.
...
### Concurrency: threads, with fiber-compatible calls

The SDK is synchronous and runs worker handlers on threads. It offers one API. Python needed an
asynchronous twin because asyncio code cannot call blocking code, and Ruby has no such split.

- The worker runs each claimed task on a thread, bounded by `concurrency` from 1 through 100.
- A dedicated thread runs heartbeat rounds on the heartbeat connection.
- A dedicated thread listens for notifications and wakes the claim loop.
- `pg` releases the GVL during network waits, so handlers that wait on I/O run in parallel. A
  CPU-bound workload runs more worker processes instead.
...
Ruby cannot stop a thread safely. `Thread#kill` and `Thread#raise` can interrupt an `ensure` block.
So the worker never kills a handler thread. Cancellation is cooperative through a token, as in the
other SDKs, and shutdown abandons a handler instead of killing it.
...
#### Recommendation: native API plus a partial adapter

Ruby ships the full native SDK above. The adapter covers the Active Job contract faithfully and adds
a few static options. Everything that changes enqueue acceptance or needs a durable context stays
native-only. Full Active Job coverage is rejected below, and so is an adapter-only SDK.

A Rails application uses both.

- **Existing Active Job jobs** keep `perform_later` and gain Workhorse storage, operator tools, and
  the dashboard.
- **Durable and cross-language work** uses `Queue#enqueue` and `Worker#handle` with JSON payloads.
  A Rails application can then enqueue a task that a Go, Python, Rust, or TypeScript worker runs,
  and run a task that another language enqueued. Active Job gives that to no other backend.

#### Adapter mapping

| Active Job                            | Workhorse                                                                                                  |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `perform_later`                       | `enqueue` of task type `active_job` with `job.serialize` as the payload                                    |
| `set(wait:)`, `set(wait_until:)`      | `enqueue_at` sets `run_at` from the `Float` epoch                                                          |
| `perform_all_later`                   | `enqueue_all` runs `enqueue_many` in atomic chunks of at most 1,000 jobs                                   |
| `queue_as`                            | `queue`                                                                                                    |
...
| `retry_on`                            | stays in Active Job. `retry_job` enqueues a new task, and the current task completes                       |
...
| continuations (`step`, `checkpoint!`) | stay in Active Job. An interrupted job resumes through `retry_job` as a new task                           |
...
One task type keeps worker registration simple. Workhorse releases an unhandled task type rather
than failing it, so a per-class task type would require every job class to be registered in advance.
The tags keep each class and each Active Job ID searchable in the dashboard.

The worker side is one call.

```ruby
worker = Stablemates::Workhorse::Worker.new(pool, queues: %w[default mailers], concurrency: 5)
Stablemates::Workhorse::ActiveJob.handle(worker)   # registers the active_job task type
Stablemates::Workhorse.run_worker_process(worker)
```

The handler sets `provider_job_id` and calls `ActiveJob::Base.execute`. The Active Job Railtie
wraps each execution in the Rails executor, so code reloading and connection release behave as in
any other backend.

#### Retries and crash recovery

Active Job owns the retries it declares. A `retry_on` match calls `retry_job`, which enqueues a new
task with the same `job_id` and an incremented `executions`. The current task then completes. So
the adapter never deduplicates on `job_id`.

Workhorse owns everything Active Job does not handle. An exception that escapes `execute` fails the
...
A job that wants Active Job to own every retry sets `max_attempts: 1`. A crash then dead-letters
the job instead of rerunning it. An operator can still redrive it from the dashboard. That matches
Solid Queue's default, and the guide presents both choices.
...
A job class may declare three static options.
...
`docs/parity.md` gains a Ruby column whose cells describe the native SDK. SM-899 adds a separate
Active Job table below the worker table. Its rows are the Client and Worker rows that the adapter
reaches, marked Supported or Native only.

| Row                                          | Through Active Job |
| -------------------------------------------- | ------------------ |
| Transactional enqueue in a caller-owned tx   | Supported          |
| Atomic batch enqueue                         | Supported          |
| Delayed enqueue (`runAt` / `run_at`)         | Supported          |
| Priority                                     | Supported          |
| Tags and max attempts                        | Supported          |
| Concurrency keys                             | Supported          |
| Enqueue trace-context propagation            | Supported          |
| Claiming and handler execution               | Supported          |
| Bounded worker concurrency                   | Supported          |
| Heartbeats, lease recovery, fenced ownership | Supported          |
| Graceful stop and signal drain               | Supported          |
| Persisted retry policies                     | Native only        |
| Absolute deadlines and execution timeouts    | Native only        |
| Enqueue idempotency                          | Native only        |
| Keyed debounce                               | Native only        |
| Keyed throttle                               | Native only        |
| Task dependencies with terminal policies     | Native only        |
| Payload and result contracts                 | Native only        |
| Recurring schedule definition sync           | Native only        |
| Cooperative cancellation delivery            | Native only        |
| Durable checkpoints (handler context)        | Native only        |
| Durable timers (`sleep` / `sleepUntil`)      | Native only        |
| Signal and human-decision waits              | Native only        |
| Linked child fan-out and result join         | Native only        |
| Latest-value progress reporting              | Native only        |
| Batch handler delivery                       | Native only        |
...
below Python's 8,242. The Active Job adapter takes about 400 of those lines. The dashboard module
...
| `active_job.rb`, `active_job/`                                 | SM-902 | adapter, handler, and `workhorse_options`    |
...
- A Rails application adopts Workhorse without rewriting its jobs, then moves individual jobs to
  the native API when it needs a durable feature.
- A Rails application can exchange tasks with workers in the other four languages through the
  native API.
...
- A job that exhausts `retry_on` without a block retries again under the Workhorse policy unless it
  sets `max_attempts`.
- Threads share the GVL, so a CPU-bound handler needs more processes rather than more
  concurrency.
...
- **Full Active Job coverage.** Idempotency and debounce would swallow `retry_job`. A durable wait
...
- **A task type per job class.** Every job class would need registration before a worker could run
  it, and an unregistered class would be released forever.
