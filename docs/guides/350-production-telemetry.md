...
When application code enqueues a task, each queue captures the active W3C trace context. PostgreSQL
stores that context beside the payload, without changing the payload your handler receives.

When a worker claims the task, `Worker` restores the stored parent before it creates the handler
