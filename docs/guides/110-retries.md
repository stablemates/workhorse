# Retries: how a failed task gets another go

When a handler throws, the task usually isn't finished — it's just failed _this_ attempt.
...
attempts. So a failed task normally goes back to `scheduled` with a wake time, not straight
to `ready`.

You can attach a retry policy to a task when you enqueue it. There are three shapes:

- **Fixed** — always wait the same amount of time.
- **Exponential** — start small, multiply the wait after each failure, stop growing at a
  ceiling.
- **Decorrelated jitter** — like exponential, but randomised, so a thousand tasks that failed
...
computed from the task's own identity and attempt number, so it's stable — replaying the same
situation picks the same delay rather than a fresh random one.

## Who actually picks the number

PostgreSQL does, from the policy stored on the task. That matters because the same policy
...
If a task has _no_ policy at all, the two paths behave differently for historical reasons:
...
The attempt counter goes up, and the task gets a new fence token when it's next claimed. But
the task id, payload, and any checkpoints you saved all survive. A retry is the same task
having another go — not a new task.
