...
SQL_STATEMENTS: dict[str, tuple[str, str]] = {
    ...
    "complete_many_and_claim_v1": (
        "SELECT * FROM workhorse.complete_many_and_claim_v1(%s::text, %s::uuid[], %s::bigint[], %s::jsonb[], %s::text, %s::integer, %s::integer)",
        "SELECT * FROM workhorse.complete_many_and_claim_v1($1::text, $2::uuid[], $3::bigint[], $4::jsonb[], $5::text, $6::integer, $7::integer)",
    ),
    "claim_v1": (
    ...
    "dashboard_human_wait_v1": (
        "SELECT task_id, queue_name, task_type, token_name AS wait_name, context, attempt, created_at, deadline_at, created_at::text AS cursor_created_at FROM workhorse.dashboard_human_wait_v1 WHERE (%s::timestamptz IS NULL OR (created_at, task_id, token_name) > (%s::timestamptz, %s::uuid, %s::text)) ORDER BY created_at, task_id, token_name LIMIT %s::integer",
    ...
    "dashboard_signal_wait_v1": (
        "SELECT task_id, queue_name, task_type, signal_name AS wait_name, attempt, created_at, deadline_at, created_at::text AS cursor_created_at FROM workhorse.dashboard_signal_wait_v1 WHERE (%s::timestamptz IS NULL OR (created_at, task_id, signal_name) > (%s::timestamptz, %s::uuid, %s::text)) ORDER BY created_at, task_id, signal_name LIMIT %s::integer",
    ...
    "enqueue_many_v1": (
        "SELECT ordinal, task_id, outcome, reason FROM workhorse.enqueue_many_v1(%s::jsonb) ORDER BY ordinal",
    ...
    "get_checkpoint": (
        ...
        "SELECT task_id::text task_id,checkpoint_name,checkpoint_value,attempt,fence_token::text fence_token,worker_id,created_at FROM workhorse.task_checkpoint WHERE task_id=$1::uuid AND checkpoint_name=$2::text",
    ),
    ...
    "list_checkpoint": (
        "SELECT checkpoint_value FROM workhorse.task_checkpoint WHERE task_id = %s::uuid AND checkpoint_name = %s::text",
        "SELECT checkpoint_value FROM workhorse.task_checkpoint WHERE task_id = $1::uuid AND checkpoint_name = $2::text",
    ),
    "list_checkpoints": (
        "SELECT task_id::text task_id,checkpoint_name,checkpoint_value,attempt,fence_token::text fence_token,worker_id,created_at FROM workhorse.task_checkpoint WHERE task_id=%s::uuid ORDER BY created_at,checkpoint_name",
        "SELECT task_id::text task_id,checkpoint_name,checkpoint_value,attempt,fence_token::text fence_token,worker_id,created_at FROM workhorse.task_checkpoint WHERE task_id=$1::uuid ORDER BY created_at,checkpoint_name",
    ),
    ...
    "list_task_timeline": (
        "SELECT kind,record_id::text record_id,priority,attempt,event_type,details,fence_token::text fence_token,worker_id,outcome,started_at,claimed_at,finished_at,error,occurred_at,has_more,cursor_occurred_at::text cursor_occurred_at FROM workhorse.list_task_timeline_v1(%s::uuid,%s::integer,%s::timestamptz,%s::text,%s::uuid)",
    ...
    "list_waits": (
        "SELECT task_id::text task_id,wait_name,mode,duration_ms::text duration_ms,requested_wake_at,wake_at,attempt,fence_token::text fence_token,worker_id,created_at FROM workhorse.task_wait WHERE task_id=%s::uuid ORDER BY created_at,wait_name",
        "SELECT task_id::text task_id,wait_name,mode,duration_ms::text duration_ms,requested_wake_at,wake_at,attempt,fence_token::text fence_token,worker_id,created_at FROM workhorse.task_wait WHERE task_id=$1::uuid ORDER BY created_at,wait_name",
    ),
    "list_workers": (
    ...
    "redrive": (
        "SELECT status,source_task_id::text source_task_id,target_task_id::text target_task_id,source_state,target_state,requested_at FROM workhorse.redrive_v1(%s::uuid,%s::text,%s::text,%s::text)",
        "SELECT status,source_task_id::text source_task_id,target_task_id::text target_task_id,source_state,target_state,requested_at FROM workhorse.redrive_v1($1::uuid,$2::text,$3::text,$4::text)",
    ),
    "redrive_many": (
    ...
    "task_checkpoint": (
        "SELECT task_id, checkpoint_name, checkpoint_value, attempt, fence_token::text,\n              worker_id, created_at\n         FROM workhorse.task_checkpoint\n        WHERE task_id = %s::uuid AND checkpoint_name = %s::text",
        "SELECT task_id, checkpoint_name, checkpoint_value, attempt, fence_token::text,\n              worker_id, created_at\n         FROM workhorse.task_checkpoint\n        WHERE task_id = $1::uuid AND checkpoint_name = $2::text",
    ),
    "task_checkpoint__checkpoints_progress_waits": (
        "SELECT task_id, checkpoint_name, checkpoint_value, attempt, fence_token::text,\n              worker_id, created_at\n         FROM workhorse.task_checkpoint\n        WHERE task_id = %s::uuid\n        ORDER BY created_at, checkpoint_name",
        "SELECT task_id, checkpoint_name, checkpoint_value, attempt, fence_token::text,\n              worker_id, created_at\n         FROM workhorse.task_checkpoint\n        WHERE task_id = $1::uuid\n        ORDER BY created_at, checkpoint_name",
    ),
    "task_progress": (
        "SELECT task_id, progress_value, revision::text, attempt, fence_token::text,\n              worker_id, created_at, updated_at\n         FROM workhorse.task_progress\n        WHERE task_id = %s::uuid",
        "SELECT task_id, progress_value, revision::text, attempt, fence_token::text,\n              worker_id, created_at, updated_at\n         FROM workhorse.task_progress\n        WHERE task_id = $1::uuid",
    ),
    "task_wait": (
        "SELECT task_id, wait_name, mode, duration_ms::text, requested_wake_at, wake_at,\n              attempt, fence_token::text, worker_id, created_at\n         FROM workhorse.task_wait\n        WHERE task_id = %s::uuid AND wait_name = %s::text",
        "SELECT task_id, wait_name, mode, duration_ms::text, requested_wake_at, wake_at,\n              attempt, fence_token::text, worker_id, created_at\n         FROM workhorse.task_wait\n        WHERE task_id = $1::uuid AND wait_name = $2::text",
    ),
    "task_wait__checkpoints_progress_waits": (
    ...
    "redrive_many_v1": (
        "SELECT status, source_task_id, target_task_id, source_state, target_state, requested_at, source_finished_at_cursor, has_more FROM workhorse.redrive_many_v1(%s::jsonb, %s::integer, %s::boolean, %s::text, %s::text, %s::text, %s::timestamptz, %s::uuid) ORDER BY ordinal",
        "SELECT status, source_task_id, target_task_id, source_state, target_state, requested_at, source_finished_at_cursor, has_more FROM workhorse.redrive_many_v1($1::jsonb, $2::integer, $3::boolean, $4::text, $5::text, $6::text, $7::timestamptz, $8::uuid) ORDER BY ordinal",
    ),
    "redrive_lineage_v1": (
    ...
    "set_queue_tier": (
        "SELECT workhorse.set_queue_tier_v1(%s::text,%s::text,%s::text,%s::text) AS tier",
        "SELECT workhorse.set_queue_tier_v1($1::text,$2::text,$3::text,$4::text) AS tier",
    ),
    "set_queue_tier_v1": (
        "SELECT workhorse.set_queue_tier_v1(%s::text, %s::text, %s::text, %s::text) AS tier",
        "SELECT workhorse.set_queue_tier_v1($1::text, $2::text, $3::text, $4::text) AS tier",
    ...
    "set_queue_history": (
        "SELECT record_attempts, record_claims FROM workhorse.set_queue_history_v1(%s::text,%s::boolean,%s::boolean)",
        "SELECT record_attempts, record_claims FROM workhorse.set_queue_history_v1($1::text,$2::boolean,$3::boolean)",
    ),
...
class StatementRegistry:
    ...
    complete_many_and_claim: DriverStatement
