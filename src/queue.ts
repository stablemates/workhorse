  ...
  async health(options: { budgets?: Partial<QueueHealthBudgets> } = {}): Promise<QueueHealth> {
    ...
    const base: Omit<QueueHealth, "status"> = {
      ...
      deadlinePressure: {
        ...
        earliestAt: row.earliest_deadline_at,
      },
      ...
      statistics: {
        rolledUpThrough: row.rolled_up_through,
        lagMs: Number(row.rollup_lag_ms),
        lastRunAt: row.last_run_at,
        buckets: Number(row.buckets),
        bucketsCapped: row.buckets_capped,
        oldestBucketAt: row.oldest_statistics_at,
        newestBucketAt: row.newest_bucket_at,
      },
      ...
      oldestRetainedAt: {
        jobIdentity: row.oldest_job_identity_at,
        terminalOutcome: row.oldest_terminal_outcome_at,
        jobEvents: row.oldest_job_event_at,
        attemptHistory: row.oldest_attempt_history_at,
        scheduleOccurrences: row.oldest_schedule_occurrence_at,
        statistics: row.oldest_statistics_at,
      },
      ...
      observations: {
        relations: relations.rows.map((relation) => ({
          ...
          lastVacuum: relation.last_vacuum,
          lastAutovacuum: relation.last_autovacuum,
