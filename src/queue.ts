...
export class Queue {
  ...
  async health(options: QueueHealthOptions = {}): Promise<QueueHealth> {
    ...
    const [exact, relations, activity, notification, retention, statistics, concurrency] =
      await Promise.all([
        this.database.query<{
    ...
    return {
      ...
      concurrencyPolicies: {
        policies: concurrency.rows.map((row) => ({
          ...
          available: Math.max(0, row.max_active - Number(row.active)),
          blockedReady: Number(row.blocked_ready),
          maxActivePerKey: row.max_active_per_key,
          saturatedKeys: Number(row.saturated_keys),
