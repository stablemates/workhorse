...
-- in one statement. It then charges each bucket once with the number of starts it admitted. When
-- the queue has no per-key rule and the window names no budget, a round skips the window and locks
-- the first ready rows directly. Both paths lock the claimed rows FOR NO KEY UPDATE, as claim_one_v1
-- does since migration 0026, so a dependent enqueue's key-share lock does not hide a ready row.
...
-- notify_concurrency_capacity_v1 published the queue for every release from a governed queue, so a
-- busy worker on a queue far below its cap took each of its own completions as a wake and delayed
-- its next claim. It now runs before the row changes and publishes only a release from a full queue
-- or a full concurrency key, which is the only release that can unblock a waiting claim.
