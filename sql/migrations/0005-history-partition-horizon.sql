...
-- History partition horizon (SM-774).

-- Preparation used to create exactly the days the health snapshot demands, so from every UTC
-- midnight until the next preparation pass the furthest demanded day did not exist and Workhorse
-- reported missing history partitions. Preparation now derives its horizon from its own cadence and
-- covers more days than the snapshot demands.
...
-- Replaced only to carry the comment that ties the snapshot's three-day window to the horizon.
...
-- Bring an existing installation up to the new horizon in the same step, so the verdict clears
-- without waiting for the next preparation pass.
