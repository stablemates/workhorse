...
-- Write full-tier enqueue rows set-based (SM-911).

-- enqueue_batch_v1 wrote each new full-tier task with its own inserts into task, task_runtime,
-- task_dependency and task_event, so a batch paid every statement and every index probe once per
-- task. The loop now validates each request, locks and reads its prerequisites, settles its
-- idempotency key and buffers its rows. After the loop, the batch writes each table with one
-- statement in input order, resolves each terminal prerequisite once, and closes every task whose
-- deadline has passed. A prerequisite always exists before the batch starts, because the batch
-- generates every new task identity. Each task keeps the order of its own events, and every
-- signature and result shape is unchanged.
