...
-- Index-pruned task lists (SM-757).

-- The two task listings read the joined task, runtime, and outcome projection and applied the tag,
-- queue, and task-type filters to the result, so a page for one tag cost what every task in the
-- installation costs. At 200,000 tasks a list filtered by one tag took 94 ms while the same tag
-- count on task_tags_gin_idx took under 4 ms.

-- Both listings now name the tasks a request can reach before the runtime and outcome joins read
-- a row. A tag filter seeks task_tags_gin_idx, and a queue or task-type filter seeks the routing
-- projection's index. Two things make that reachable: the tag filter arrives through an immutable
-- function, which the planner evaluates while it plans rather than once per execution, and each
-- pruning branch tests only the request, so the planner folds away the branches this request did
-- not ask for. Every filter still applies to the result, so a request that combines filters keeps
-- all of them.
