...
-- A dependent enqueue holds its prerequisites against completion (SM-914).

-- enqueue_batch_v1 locked each prerequisite task row FOR UPDATE before it read the prerequisite's
-- outcome and wrote the dependency edge. That lock kept a concurrent completion from resolving
-- dependents on a snapshot without the new edge, but it also conflicted with the key-share lock
-- that every foreign key check takes on the task row. A claim, which inserts a task event, therefore
-- waited for the whole enqueue transaction of any dependent.

-- The enqueue now takes key-share locks, first on the prerequisites' task_runtime rows and then on
-- their task rows. Every terminal transition deletes the runtime row before it records the outcome
-- that resolves dependents, and that delete waits for the key-share lock. The transition's resolver
-- then runs after the edge commits and sees it. A transition that committed first has already
-- deleted the row, and the enqueue's outcome reads see its outcome. The runtime rows come first
-- because completion and purge lock runtime before task, so neither side can hold one row while it
-- waits for the other.

-- claim_one_v1 now locks its candidate FOR NO KEY UPDATE instead of FOR UPDATE. The claim changes
-- no key column, and FOR UPDATE would make SKIP LOCKED pass over a ready prerequisite while any
-- dependent enqueue held it. Every signature and result shape is unchanged.
